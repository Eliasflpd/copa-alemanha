...
export default function Home() {
  ...
  useEffect(() => {
    const handleBeforeUnload = (e: BeforeUnloadEvent) => {
      if (appStep === "loading-generate") e.preventDefault();
    };
  ...
  const handleQuizNext = useCallback(() => {
    if (quizStep === 1) {
      ...
      setTimeout(() => { setQuizStep(2); setAppStep("quiz-2"); }, 3000);
    } else if (quizStep === 2) {
      setQuizStep(3);
      setAppStep("quiz-3");
    } else if (quizStep === 3) {
  ...
  const handleQuizBack = useCallback(() => {
    if (quizStep === 2) { setQuizStep(1); setAppStep("quiz-1"); }
    else if (quizStep === 3) { setQuizStep(2); setAppStep("quiz-2"); }
  }, [quizStep]);

  return (
    <main className="flex flex-col items-center min-h-screen bg-de-gold">
      {appStep === "hero" && (
        <Hero onStart={() => {
